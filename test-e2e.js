...
async function testFlightSearch() {
  ...
  try {
    ...
    const { response, data } = await makeRequest(`/api/amadeus/flights/search?${searchParams}`);

    if (response.ok && data.data && Array.isArray(data.data)) {
      console.log(`✅ Flight search successful - found ${data.data.length} flights`);
...
async function testBookingCreation(selectedFlight) {
  ...
  try {
    ...
    if (response.ok && data.success && data.booking) {
      ...
      console.log(`   Amount: ${data.booking.currency}${data.booking.totalAmount}`);
      return data.booking;
    }
